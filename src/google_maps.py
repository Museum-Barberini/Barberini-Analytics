...
class FetchGoogleMapsReviews(luigi.Task):
    ...
    def fetch_raw_reviews(self, service, page_size=100):
        ...
        try:
            print("Fetched {len(reviews)} out of {total_reviews} reviews", end='', flush=True)

            while 'nextPageToken' in review_list:
            # TODO: optimise by requesting the latest review from DB and not fetching more pages once that one is found
