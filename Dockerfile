...
RUN apt-get update
ARG apt-install='apt-get install -y --no-install-recommends'
...
# instal utilities
RUN apt-install build-essential vim curl gnupg iproute2 nano psmisc

# install python
RUN apt-install python3.6 python3-pip python3-setuptools python3-dev python3-wheel
...
RUN DEBIAN_FRONTEND=noninteractive apt-install libpq-dev
